...
with st.sidebar:
    ...
    st.info("Modèle : llama-3.1-8b-instant")
...
# 7. LOGIQUE DE CALCUL (SCORE2 & Guidelines SFC)
...
if st.button("Générer la lettre (Gratuit avec Groq)"):
    if not api_key_groq:
        ...
        try:
            ...
            prompt = f"Rédige une lettre médicale de l'Alliance Protectrice pour {nom_patient}, {age} ans, risque {st.session_state.cat_risque}. Inclure sport >= 150min, arrêt tabac ({fumeur}) et alimentation saine."
            ...
            st.download_button("📥 Télécharger la lettre", lettre, file_name=f"Lettre_{nom_patient}.txt")
